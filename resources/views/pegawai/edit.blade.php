<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <h1 class="text-2xl font-semibold text-gray-800">Edit Pegawai</h1>
      ...
      <div class="overflow-x-auto w-full">
        <form action="{{ route('pegawai.update', $pegawai->id) }}" method="POST" enctype="multipart/form-data"
          ...
          <div class="mt-4">
            ...
            <textarea id="alamat" name="alamat" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" required>{{ old('alamat', $pegawai->alamat) }}</textarea>
          ...
          <div class="grid grid-cols-2 gap-4 mt-4">
            ...
            <div>
              ...
              <select id="bagian_id" name="bagian_id" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm"
                required>
                <option value="">Pilih Bagian</option>
                @foreach ($bagians as $bagian)
                  <option value="{{ $bagian->id }}"
                    {{ old('bagian_id', $pegawai->bagian_id) == $bagian->id ? 'selected' : '' }}>
                    {{ $bagian->nama_bagian }}
                  </option>
                @endforeach
              </select>
          ...
          <div class="grid grid-cols-2 gap-4 mt-4">
            <div>
              ...
              <select id="jabatan_id" name="jabatan_id" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm"
                required>
            ...
            <div>
              ...
              @if ($pegawai->foto)
                <div class="mt-2">
                  <img src="{{ asset('storage/' . $pegawai->foto) }}" alt="Foto Pegawai" class="w-32 h-32 rounded-md">
                </div>
              @endif
            </div>
          </div>
