<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <h1 class="text-2xl font-semibold text-gray-800">Edit Tanaman</h1>

    <div class="overflow-hidden mb-8 w-full rounded-lg shadow-xs">
      <div class="overflow-x-auto w-full">
        <form action="{{ route('tanaman.update', $tanaman->id) }}" method="POST" autocomplete="off">
          @csrf
          @method('PUT')
          ...
          <div class="mt-4">
            ...
            <select name="lahan_id" id="lahan_id"
              ...
              @foreach ($lahans as $lahan)
                <option value="{{ $lahan->id }}" {{ $tanaman->lahan_id == $lahan->id ? 'selected' : '' }}>
                  {{ $lahan->nama_lahan }}</option>
              @endforeach
          ...
          <div class="mt-4">
            ...
            <x-text-input type="date" id="tanggal_tanam" name="tanggal_tanam" class="block w-full"
              value="{{ old('tanggal_tanam', $tanaman->tanggal_tanam) }}" required />
